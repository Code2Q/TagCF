...
class Dataset(torch.utils.data.Dataset):
        ...
        for field in columns:
            ...
            if ftype == FeatureType.TOKEN_SEQ:
                df[field] = [
                    ...
                    np.array(list(map(int, filter(None, _.split(seq_separator)))))
    ...
    def _get_remap_list(self, field_list):
        ...
        for field in field_list:
            if field == 'user_tag_id_list' or field == 'item_tag_id_list':
                continue
            ftype = self.field2type[field]
            for feat in self.field2feats(field):
                remap_list.append((feat, field, ftype))
            print(f"===remap list :==== {remap_list}")
        return remap_list
    ...
    def _remap(self, remap_list):
        ...
        for (feat, field, ftype), new_ids in zip(remap_list, new_ids_list):
            # if feat == 'utid2itid_feat':
            # print(feat, field, ftype, new_ids)
    ...
    def split_by_time_field(self, time_field, time_value=1725292318498):
        """Split interaction records by a specific time value.

        Args:
            time_field (str): The time field used for splitting the interactions.
            time_value (int): The specific time value for splitting.

        Returns:
            tuple: A tuple containing the training set and validation set.
        """
        self.logger.debug(f"split by time field [{time_field}], time value [{time_value}]")
        print("@@@@@@@@@@@@@@@@@@@@@@")
        print(self.inter_feat[time_field].max())
        print(self.inter_feat[time_field].min())

        # Split the data based on the time value
        train_mask = self.inter_feat[time_field] <= time_value
        valid_mask = self.inter_feat[time_field] > time_value

        self._drop_unused_col()

        train_df = self.inter_feat[train_mask]
        valid_df = self.inter_feat[valid_mask]
        test_df =  []  # Test set is empty

        
        # Create new Dataset objects
        train_dataset = self.copy(train_df)
        valid_dataset = self.copy(valid_df)
        # test_dataset = None
    ...
    def build(self):
        ...
        if split_mode == "RS":
            ...
            if group_by is None or group_by.lower() == "none":
                ...
                print(f"==== split_by_time_field ====")
                datasets = self.split_by_time_field(time_field=self.time_field)
            elif group_by == "user":
    ...
    def _dataframe_to_interaction(self, data):
        ...
        for k in data:
            ...
            if ftype == FeatureType.TOKEN:
                ...
                # Ensure each sequence is a sequence of integers
                seq_data = [torch.LongTensor(np.array(d, dtype=np.int64)[:self.field2seqlen[k]]) for d in value]
