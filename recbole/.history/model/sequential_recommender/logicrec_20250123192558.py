...
class LogicRec(SequentialRecommender):
    def __init__(self, config, dataset):
        ...
        self.ut2it = torch.load('u2i.pt').to(self.device)
        self.it2ut = torch.load('i2u.pt').to(self.device)
    ...
    def tag_infer_train(self, target_tags): 
    ...
    def calculate_loss(self, interaction):
        ...
        if self.mode == 'item_tag':
            ...
            all_tag_embds = self.user_tag_embeddings(all_target_tags) 
    ...
    def predict(self, interaction):
        ...
        target_tags = self.item_feat[self.tag_col_name][test_item] 
        if self.mode == 'item_tag':
            all_tag_emb = self.item_tag_embeddings(target_tags)
        else:
            all_tag_emb = self.user_tag_embeddings(target_tags)

        id_scores = torch.mul(output, test_item_emb).sum(dim=1)  
        tag_scores = torch.mul(output.unsqueeze(1), all_tag_emb).sum(dim=-1)  
    ...
    def infer_tags(self, all_tag_scores):
        if self.mode == 'user_tag':
            ...
            topk_ut_values, topk_ut_indices = torch.topk(all_tag_scores, self.branch_factor, dim=1) 
            topk_ut_values /= (topk_ut_values.sum(dim=1, keepdim=True) + 1e-8)

            selected_ut2it = self.ut2it[topk_ut_indices] 
            topk_infer_user_tags_expanded = topk_ut_values.unsqueeze(-1) 
            aggregated_item_tags = torch.bmm(selected_ut2it.transpose(1, 2), topk_infer_user_tags_expanded).squeeze(-1) 
            topk_infer_item_tags, topk_it_indices = torch.topk(aggregated_item_tags, self.branch_factor, dim=-1) 
            topk_infer_item_tags /= (topk_infer_item_tags.sum(dim=1, keepdim=True) + 1e-8)

            selected_it2ut = self.it2ut[topk_it_indices] 
            topk_infer_item_tags_expanded = topk_infer_item_tags.unsqueeze(-1) 
            aggregated_user_tags = torch.bmm(selected_it2ut.transpose(1, 2), topk_infer_item_tags_expanded).squeeze(-1)  
            ...
            topk_it_values, topk_it_indices = torch.topk(all_tag_scores, self.branch_factor, dim=1) 
    ...
    def full_sort_predict(self, interaction):
        ...
        if self.full_tag_infer:
            if self.mode == 'user_tag':
                infer_item_tags = torch.matmul(all_tag_scores_sig, self.ut2it) 
                ...
                infer_user_tags = torch.matmul(all_tag_scores_sig, self.it2ut) 
                infer_user_tags /= (infer_user_tags.sum(dim=1, keepdim=True) + 1e-8)
                infer_tags = torch.matmul(infer_user_tags, self.ut2it)
                infer_tags /= (infer_tags.sum(dim=1, keepdim=True) + 1e-8)
            weighted_emb = (all_tag_emb.unsqueeze(0) * infer_tags.unsqueeze(-1) ).sum(dim=1)
            infer_average_score = torch.matmul(weighted_emb, test_items_emb.t()) 
        
        else:
            infer_tags_freq, infer_tags_indices = self.infer_tags(all_tag_scores_sig) 
            selected_tag_emb = all_tag_emb[infer_tags_indices] 
        ...
        predict_tag_scores = selected_scores.mean(dim=2)  + self.tag_decay * infer_average_score


        return id_scores + predict_tag_scores
        # return id_scores s