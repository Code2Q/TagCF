...
class MultiHeadCrossAttention(nn.Module):
    ...
    Args:
        query_tensor (torch.Tensor): the input tensor for queries
        key_value_tensor (torch.Tensor): the input tensor for keys and values
        attention_mask (torch.Tensor): the attention mask for key_value_tensor

    Returns:
        hidden_states (torch.Tensor): the output of the multi-head cross-attention layer
    """
    ...
    def forward(self, query_tensor, key_value_tensor, attention_mask):
        ...
        mixed_key_layer = self.key(key_value_tensor)
        mixed_key_layer = mixed_key_layer.view(mixed_key_layer.shape[0], -1, mixed_key_layer.shape[-1])
        mixed_value_layer = self.value(key_value_tensor)
        mixed_value_layer = mixed_value_layer.view(mixed_value_layer.shape[0], -1, mixed_value_layer.shape[-1])
        # print(mixed_query_layer.shape, mixed_key_layer.shape) #torch.Size([1, 50, 128]) torch.Size([1, 50, 15, 128])     

        # Transpose for scores
        query_layer = self.transpose_for_scores(mixed_query_layer)  # [batch_size, num_heads, seq_len_q, head_dim]
        key_layer = self.transpose_for_scores(mixed_key_layer)  # [batch_size, num_heads, seq_len_k, head_dim]
        value_layer = self.transpose_for_scores(mixed_value_layer)  # [batch_size, num_heads, seq_len_v, head_dim]

        # Compute attention scores
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))  # [batch_size, num_heads, seq_len_q, seq_len_k]
        attention_scores = attention_scores / self.sqrt_attention_head_size

        # Adjust attention mask shape
        if attention_mask is not None:
            # attention_mask should be [batch_size, 1, 1, seq_len_k] to broadcast correctly
            attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)  # [batch_size, 1, 1, seq_len_k]
            attention_scores = attention_scores + attention_mask
        ...
        context_layer = torch.matmul(attention_probs, value_layer)  # [batch_size, num_heads, seq_len_q, head_dim]
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()  # [batch_size, seq_len_q, num_heads, head_dim]
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)  # [batch_size, seq_len_q, hidden_size]
